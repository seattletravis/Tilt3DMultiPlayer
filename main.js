...
physicsWorld.defaultContactMaterial.contactEquationStiffness = 100000000; //default 50,000,000
...
function moveCameraUp() {
	...
	camera.lookAt(0, camera.position.y + camPosLookDif, 0);
	buttonUp.className =
		'text-green-600 border-4 border-green-600  bg-blue-600 inline-block py-1 rounded-full px-8';
}
...
function moveCameraDown() {
	...
	buttonDown.className =
		'text-green-600 border-4 border-green-600  bg-blue-600 inline-block px-4 py-1 rounded-full px-4';
}
...
function moveCameraLeft() {
	...
	buttonLeft.className =
		'text-green-600 border-4 border-green-600  bg-blue-600 inline-block px-4 py-1 rounded-full px-6';
}
...
const btnDown =
	'text-green-600 border-4 border-green-600 bg-yellow-400 inline-block py-1 rounded-full px-4';
const btnLeft =
	'text-green-600 border-4 border-green-600 bg-yellow-400 inline-block py-1 rounded-full px-6';
const btnRight =
	'text-green-600  border-4 border-green-600 bg-yellow-400 inline-block py-1 rounded-full px-4';
const btnUp =
	'text-green-600  border-4 border-green-600 bg-yellow-400 inline-block py-1 rounded-full px-8';
const btnIn =
	'text-green-600  border-4 border-green-600 bg-yellow-400 inline-block py-1 rounded-full px-8';
const btnOut =
	'text-green-600  border-4 border-green-600 bg-yellow-400 inline-block py-1 rounded-full px-8';
const btnEnter =
	'text-green-600 bg-yellow-400 font-bold text-center border-4 border-green-600 inline-block text-xl px-4 rounded-full';
const btnGithub =
	'text-green-600 bg-yellow-400 font-bold text-center border-4 border-green-600 inline-block px-4 rounded-full';
...
function blueGithub() {
	githubButton.className =
		'text-green-600 bg-blue-600 font-bold text-center border-4 border-green-600 inline-block px-4 rounded-full';
}
...
//Key Light
const gameLight = new THREE.PointLight(0xffffff, 0.3, 2000);
gameLight.castShadow = true;
gameLight.position.set(-3, 1, 3);
scene.add(gameLight);

//Fill Light
const fillLight = new THREE.PointLight(0xffffff, 0.1, 2000);
...
redDrop.position.set(
	scoreRadialDistance * Math.cos(redAngle),
	scoreBallY,
	scoreRadialDistance * Math.sin(redAngle)
);
scene.add(redDrop);
let blueDrop = new THREE.Mesh(dropSphereGeopmetry, dropBlueSphereMaterial);
blueDrop.position.set(
	scoreRadialDistance * Math.cos(blueAngle),
	scoreBallY,
	scoreRadialDistance * Math.sin(blueAngle)
);
scene.add(blueDrop);
...
const topBlockTexture = new THREE.TextureLoader().load(
	'./tower_images/wood.jpg'
);
const mass = 0.00001;
...
const topBlockMesh = new THREE.Mesh( //visual part of ground
	new THREE.CylinderGeometry(0.75, 0.75, 0.25, 25),
	new THREE.MeshStandardMaterial({
		map: topBlockTexture,
	})
);
...
function getCenterOfTopBlock() {
	...
	if (colorOffsetRed > 255) {
		colorOffsetRed = 255;
	}
	if (colorOffsetGreen < 0) {
		colorOffsetGreen = 0;
	}
	dangerMeterText.style.color =
		'rgb(' + colorOffsetRed + ',' + colorOffsetGreen + ', 0';
...
for (let i = 0; i <= 17; i++) {
	//use i <= 17 for 54 blocks
	let blockLayer = i;
	let PosY = i * 0.3 + 0.02 - 10;
	if (blockLayer % 2 == 0) {
		let randomBlockName1 = 'logo' + Math.ceil(Math.random() * 12);
		const logo1 = new THREE.TextureLoader().load(
			'./tower_images/' + randomBlockName1 + '.png'
		);
		let randomBlockTopBottom1 = 'topBottomA' + Math.ceil(Math.random() * 10);
		const topBottomBlock1 = new THREE.TextureLoader().load(
			'./tower_images/' + randomBlockTopBottom1 + '.png'
		);
		let randomBlockName2 = 'logo' + Math.ceil(Math.random() * 12);
		const logo2 = new THREE.TextureLoader().load(
			'./tower_images/' + randomBlockName2 + '.png'
		);
		let randomBlockTopBottom2 = 'topBottomA' + Math.ceil(Math.random() * 10);
		const topBottomBlock2 = new THREE.TextureLoader().load(
			'./tower_images/' + randomBlockTopBottom2 + '.png'
		);
		let randomBlockName3 = 'logo' + Math.ceil(Math.random() * 12);
		const logo3 = new THREE.TextureLoader().load(
			'./tower_images/' + randomBlockName3 + '.png'
		);
		let randomBlockTopBottom3 = 'topBottomA' + Math.ceil(Math.random() * 10);
		const topBottomBlock3 = new THREE.TextureLoader().load(
			'./tower_images/' + randomBlockTopBottom3 + '.png'
		);
...
function explodeTower() {
	...
	for (let i = 0; i < blockPhysicsArray.length; i++) {
		let randoX = (Math.random() - 0.5) * 0.0005;
		let randoZ = (Math.random() - 0.5) * 0.0005;
		let randoY = Math.random() * 0.000005;
		blockPhysicsArray[i].applyImpulse(
			new CANNON.Vec3(randoX, randoY, randoZ),
			new CANNON.Vec3(0, 0, 0)
		);
	}
	topBlock.applyImpulse(
		new CANNON.Vec3(
...
function getHitPoint(clientX, clientY, mesh, camera) {
	...
	mouse.x =
		((clientX - sidePanel.offsetWidth) / canvasContainer.offsetWidth) * 2 - 1;
...
window.addEventListener('pointerdown', (event) => {
	clickMouse.x =
		((event.clientX - sidePanel.offsetWidth) / canvasContainer.offsetWidth) *
			2 -
		1;
...
window.addEventListener('pointerup', (event) => {
	...
	if (holdingTile == true) {
		...
		if (redDroppability == true && redsTurn == true && gameOver == false) {
			...
			blueScore.className =
				'm-auto text-blue-900 text-2xl font-bold border-4 border-blue-900 rounded-full px-2';
			dropBlueSphereMaterial.opacity = 1;
			dropRedSphereMaterial.opacity = 0.3;
		...
		if (blueDroppability == true && redsTurn == false && gameOver == false) {
			...
			redScore.className =
				'm-auto text-red-600 text-2xl font-bold border-4 border-red-600 rounded-full px-2';
			dropBlueSphereMaterial.opacity = 0.2;
